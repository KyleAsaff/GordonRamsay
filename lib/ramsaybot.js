...
RamsayBot.prototype._getZeroCaterMenu = function (message, day) {
	...
	var resp = "*"+date+"*\n\n";
	requestify.get('https://zerocater.com/m/TQGB').then(function(response) {
...
RamsayBot.prototype._getZeroCaterMenuItems = function (url) {
	var that = this;
	request(url, function (error, response, html) {
		if (!error && response.statusCode == 200) {
			var $ = cheerio.load(html);
			var title = $('.meal-title').text();
			console.log(title);
		} else {
			resp = "I couldn't find an avaliable menu for "+date+". Please try again later.";
			that.postMessage(message.channel, resp, {as_user: true});
