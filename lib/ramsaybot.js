...
RamsayBot.prototype._onMessage = function (message) {
    if (this._isChatMessage(message) && this._isChannelConversation(message) && !this._isFromRamsayBot(message) && this._isMentioningRamsay(message)) {
    	if(this._isMentioningFoodTrucks(message)) {
    		...
    		this._getTruckIdFromArray(truck);
    	}
...
RamsayBot.prototype._getTruckIdFromArray = function (truckName) {
	var len = this.trucks.length
	if(len > 0) {
		for(var i = 0; i<len; i++) {
			if(truckName.toLowerCase() === this.trucks[i].name) {
				this._passFoodTruckMenus(this.trucks[i].id, this.trucks[i].name);
				break;
			}
			if(i===len-1) {
				console.log("There is no truck with the name "+truckName+" for "+this.trucks[i].day);
			}
		}
	} else {
		console.log("Ask me about which trucks are avaliable first!");
	}
}

RamsayBot.prototype._passFoodTruckMenus = function (id, truckName) {
	request.post('https://www.gfoodtrucks.com/ptgettruckproductajaxlist', {form: {truckId:id,schedule:'0',forceclose:'0',curTab:'0'}}, function (error, response, html) {
		if (!error && response.statusCode == 200) {
	    	var $ = cheerio.load(html);
	    	if($('h3').length > 0) {
	    		...
	    		console.log("No menu avaliable for "+truckName);
	    	}
		}
	});
};

RamsayBot.prototype._passFoodTrucks = function (html, tab, day) {
	var that = this;
	that.trucks = [];
	...
	request('https://www.gfoodtrucks.com/', function (error, response, html) {
		if (!error && response.statusCode == 200) {
	    	...
	    	if($(tab+' div.ordering_box').length === 0) {
	    		console.log("There is no food trucks for "+day);
	    	}
	    	else {
		    	$(tab+' div.ordering_box').each(function(i, element){
		    		console.log("called");
		    		...
		    		console.log("Description: "+desc);
		    		console.log("id: "+id);
		    		that.trucks.push({tab:tab, id:id, name:name.toLowerCase(), desc:desc, day:day});
		    		//console.log(that.trucks);
		    	});
	    	}
		}
	});
		...
		if (!error && response.statusCode == 200) {
	    	...
	    	$('.nav.nav-tabs > li > a').each(function(i, element){
	    		if($(this).text() === day) {
	    			...
	    			console.log(tab);
	    			that._passFoodTrucks(html, tab, day);
	    		}
	    	});
		}
	});
