...
RamsayBot.prototype._onMessage = function (message) {
    ...
    if (this._isChatMessage(message) && this._isChannelConversation(message) && !this._isFromRamsayBot(message) && this._isMentioningRamsay(message)) {
    	...
    	} else if (this._isMentioningZeroCater) {
    		var day = this._getZeroCaterDay(message);
			this.postMessage(message.channel, day, {as_user: true});
    	}
    }
};
...
RamsayBot.prototype._getZeroCaterDay = function (message) {
	var day;
	var date = Date.today().toLongDateString();
	// get next menu:
	// if date is fri sat sun mon, get next tuesday date
	// if date is tuesday get todays date
	// if date is 
	
	// get menu tuesday
	// if date is tuesday, get todays date
	// if date not tuesday, get next tuesday date
	// 
	// get menu thursday
	// if date is thursday, get todays date
	// if date is not thursday get next thursday date
	if(message.text.toLowerCase().indexOf('mon') > -1 || message.text.toLowerCase().indexOf('monday') > -1) {
 		day = "mon";
 		...
 		if(date.toLowerCase().indexOf("tues") > -1 || date.toLowerCase().indexOf("wed") > -1) {
 			return Date.today().next().thursday().toString('yyyy-MM-dd');
 		} else {
 			return Date.today().next().monday().toString('yyyy-MM-dd');
 		}
