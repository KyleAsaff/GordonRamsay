...
RamsayBot.prototype._onMessage = function (message) {
    ...
    if (this._isChatMessage(message) && this._isChannelConversation(message) && !this._isFromRamsayBot(message) && this._isMentioningRamsay(message)) {
    	if(this._isMentioningHelp(message)) {
    		...
    		day = this._getZeroCaterDay(message);
...
RamsayBot.prototype._getZeroCaterDay = function (message) {
	var day;
	var date = new Date.today().addHours(-8).toString();

	if(message.text.toLowerCase().indexOf('mon') > -1) {
 		day = "mon";
 		if(date.toLowerCase().indexOf(day) > -1) {
 			return Date.today().addHours(-8).toString('yyyy-MM-dd');
 		} else {
 			// tuesday because 6PM MON -> GMT timezone
 			return Date.today().addHours(-8).next().tuesday().toString('yyyy-MM-dd');
 		}
 	} else if(message.text.toLowerCase().indexOf('thu') > -1) {
 		day = "thu";
 		if(date.toLowerCase().indexOf(day) > -1) {
 			return Date.today().addHours(-8).toString('yyyy-MM-dd');
 		} else {
 			return Date.today().addHours(-8).next().thursday().toString('yyyy-MM-dd');
 		}
 	} else {
 		if(date.toLowerCase().indexOf("mon") > -1) {
 			return Date.today().addHours(-8).toString('yyyy-MM-dd');
 		} if(date.toLowerCase().indexOf("thu") > -1) {
 			return Date.today().addHours(-8).toString('yyyy-MM-dd');
 		} else if(date.toLowerCase().indexOf("tue") > -1 || date.toLowerCase().indexOf("wed") > -1) {
 			return Date.today().addHours(-8).next().thursday().toString('yyyy-MM-dd');
 		} else {
 			// tuesday because 6PM MON -> GMT timezone
 			return Date.today().addHours(-8).next().tuesday().toString('yyyy-MM-dd');
...
RamsayBot.prototype._passFoodTrucks = function (message, html, tab, day) {
	...
	if($(tab+' div.ordering_box').length === 0) {
    	...
    	$(tab+' div.ordering_box').each(function(i, element){
    		...
    		if($(this).find('.mask').attr('id')) {
    			id = $(this).find('.mask').attr('id').split('_')[1];
    		}
...
RamsayBot.prototype._getZeroCaterMenu = function (message, day) {
	...
	var pstDate = Date.parse(day).addDays(-1).toString('dddd, MMMM dd, yyyy');
	var resp = "";
	request('https://zerocater.com/m/TQGB', function (error, response, html) {
		if (!error && response.statusCode == 200) {
	    	var $ = cheerio.load(html);
	    	var monthMenuFormat = Date.parse(day).toString('yyyyMM');
	    	var monthMenu = $('div.meal-month[data-month="'+monthMenuFormat+'"]').children();
	    	if(monthMenu.length > 0) {
	    		monthMenu.each(function(i, element){
	    			var menuDateFormat = Date.parse(day).toString('yyyy-MM-dd');
	    			var elementDate = element.attribs['data-reactid'].split('$')[2];
	    			if(menuDateFormat === elementDate) {
	    				var href = $(this).find('a.meal-preview').attr('href');
	    				var url = 'https://zerocater.com'+href;
	    				that._getZeroCaterMenuItems(message, url, pstDate);
	    			}
	    		});
	    	}
	    	else {
			...
			resp = "I couldn't find an avaliable menu for "+pstDate+". Please try again later.";
			that.postMessage(message.channel, resp, {as_user: true});
		}
	});
};

RamsayBot.prototype._getZeroCaterMenuItems = function (message, url, date) {
	var that = this;
	var resp = "*"+date+"*\n\n";
	request(url, function (error, response, html) {
		if (!error && response.statusCode == 200) {
			var $ = cheerio.load(html);
			var mealType = $('.meal-title').text().trim().toUpperCase();
			resp = resp+"*"+mealType+"*\n";
			var vendor = $('.meal-details').text().trim();
			var vendorDesc = $('.vendor-description').text().trim();
			resp = resp+vendor+" - "+vendorDesc+"\n\n";
			resp = resp+"*MENU*\n\n";
			mealItems = $('.meal-items').children();
	    	mealItems.each(function(i, element){
	    		var name = $(this).find('.name').text().trim();
	    		var desc = $(this).find('.description').text().trim();
	    		var dietary = $(this).find('.dr-labels').children();
	    		var labels = "_(";
	    		dietary.each(function(i, element) {
	    			var label = $(this).text().trim();
	    			labels = labels+""+label+" ";
	    		});
	    		labels = labels.trim();
	    		labels = labels+")_";
	    		resp = resp+"*"+name+"* "+labels;
	    		if(desc.length > 0) {
	    			resp = resp+"\n"+desc;
	    		}
	    		resp = resp+"\n";
